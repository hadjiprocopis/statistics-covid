...
with socio-economical data a sub-package (L<Statistics::Covid::WorldbankData>)
...
=head1 SYNOPSIS
	...
	$objs =
	  $covid->select_datums_from_db_time_ascending(
		'conditions' => {
			#'admin1' =>{'like' => 'Ha%'}, # the location (wildcard)
			#'admin1' =>['Halton', 'Havering'],
			'admin0' => 'UK', # the admin0 (could have been wildcarded or omitted)
	  );
	...
	my $objs = $io->db_select({
		conditions => {'admin0'=>'UK', 'admin1'=>{'like' => 'Ha%'}}
	});
