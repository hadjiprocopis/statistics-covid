package Statistics::Covid::DataProvider::World::JHUgithub;
...
sub new {
	...
	$params->{'repository-github'} = [
		...
		# DateTime->now()->mdy().'.csv', # e.g. 04-01-2020.csv
	...
	$self->name('World::JHUgithub'); # <<<< Make sure this is unique over all providers
...
sub create_data_id {
	my $self = $_[0];
	# the input is an arrayref of [url, data_received_string, data_as_perlvar]
	# it is not an array of those
	my $datas_item = $_[1];
	...
	for($i=scalar(@{$apv});$i-->1;){
		...
		if( ! defined $dt ){
			...
			# in postprocess!
			warn pp($apv->[$i])."\nwarning, call to ".'JHU_datestring_to_DateTime()'." has failed for 'last_update' value of '".$apv->[$i]->{'last_update'}."' for above data, from url '$aurl', will skip this BUT IT SHOULD HAVE BEEN CAUGHT in postprocess_fetched_data";
			next
		}
		$epochsecs = $dt->epoch();
	...
	if( ! defined($dt=Statistics::Covid::Utils::JHU_datestring_to_DateTime($epochsecs)) ){
		warn pp($apv->[$i])."error, failed to parse date '$epochsecs' from input json data just transfered from url '$aurl'.";
		return undef;
	}
...
sub postprocess_fetched_data {
	...
	my $datas = $_[1];
        ...
        # FIPS,Admin2,Province_State,Country_Region,Last_Update,Lat,Long_,Confirmed,Deaths,Recovered,Acti$
	...
	for my $adatas (@$datas){
		...
		if( ! defined $data_id ){ warn "error, call to ".'create_data_id()'." has failed for '".$adatas->[0]."'"; return 0 }
		$adatas->[3] = $data_id;
...
sub create_Datums_from_fetched_data {
	...
	my ($admin0, $admin1, $admin2, $admin3, $admin4,
	    ...
	    $id, $lat, $long, $official_name, $pv, $province_state,
	    $country_region, $type, $confirmed, $recovered, $deaths, $m,
	    $incidentrate, $peopletested
	);
	my $ds = $self->name();
	for my $adata (@$datas){
		#my $aurl = $adata->[0];
		#my $content = $adata->[1];
		my $pv = $adata->[2]; # getting to the array of locations
...
# there must be one inner item for each remote file read
...
# returns undef on failure or the ARRAY of outfiles (one for each saved file)
# if successful, so each remote file is saved separately
