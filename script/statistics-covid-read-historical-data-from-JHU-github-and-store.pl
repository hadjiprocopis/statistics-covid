...
die usage() . "\n\nA provider (--provider) is required, use 'World::JHUgithub' for remote github repo of JHU and/or 'World::JHUlocaldir' (if remote git was transfered locally (which is the preferred way as there is a limit on the number of github-downloads)." if scalar(@providers)==0;
