			...
			'cb-postprocess' => sub {
				...
				for my $ablobs ($root->children()){
					for my $ablob ($ablobs->children()){
						# these idiots include all kind of files in there, even markdown!
						if( defined($m=$ablob->first_child('Name'))
						 && ($m->text =~ /data_\d+\.json$/i)
						# && ($m->text =~ /(?!geo).*?\.json$/i)
						){
							$fname = $m->text;
							print __FILE__." found json data : $fname\n";
						} else { next }
						if( defined($m=$ablob->first_child('Properties'))
							...
							if( (! $latest_date) || ($latest_date < $adateobj) ){
				...
				if( ! defined $latest_date ){ warn pp($data)."\nerror, failed to find valid latest date for above data"; return undef }
				if( ! defined $latest_fname ){ warn pp($data)."\nerror, failed to find valid latest filename for above data"; return undef }
				my $aurl = 'https://c19pub.azureedge.net/'.$latest_fname;
				# modify the url of the next entry
				$self->{'urls'}->[1]->{'url'} = $aurl;
				warn "fetched and parsed the entry point url, data file last updated on $latest_date is at '$aurl'.\n";
...
sub create_data_id {
	...
	my $aurl = $data->[0];
	my $apv = $data->[2];
	my $date_str = $apv->{'lastUpdatedAt'} // $apv->{'Last-Modified'} // $apv->{'last-updated'};
	if( ! defined $date_str ){ warn pp($apv)."\nerror, there is no 'lastUpdatedAt' or 'Last-Modified' in the above data"; return undef }
...
sub postprocess_fetched_data {
	...
	my $pv = $datas->[$I]->[2];
	if( ! defined $pv ){
		# if not already converted in base
		$pv = Data::Roundtrip::json2perl($datas->[$I]->[1]);
		if( ! defined $pv ){
			warn "it is not json, trying YAML";
			$pv = Data::Roundtrip::yaml2perl($datas->[$I]->[1]);
		}
		if( ! defined $pv ){
			warn $datas->[$I]->[1]."\nabove content is neither json nor YAML, will save as text";
			$pv = $datas->[$I]->[1];
		}
		$datas->[$I]->[2] = $pv;
	}

	# there is a lastUpdatedAt or 'Last-Modified' entry in the 2nd data (main data).
	# but also the first data contains last-update date in epoch seconds
	my $dataid = $self->create_data_id($datas->[$I]);
...
sub create_Datums_from_fetched_data {
	...
	# it is like: 2020-04-16T14:44:18.371573Z
	# so remove last millis
	$adatestr = $datas->[$I]->[2]->{'lastUpdatedAt'} // $datas->[$I]->[2]->{'Last-Modified'};
	if( ! defined $adatestr ){ warn pp($datas->[$I]->[2])."\nerror, there is no 'lastUpdatedAt' or 'Last-Modified' in the above data"; return undef }
	# it is like: 2020-04-16T14:44:18.371573Z
	# so remove last millis
	$adatestr =~ s/\.\d+//;
