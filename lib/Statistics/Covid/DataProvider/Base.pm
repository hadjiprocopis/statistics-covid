...
sub	_fetch_from_urls {
	...
	for my $anentry (@{$self->{'urls'}}){
		...
		if( defined($cb=$anentry->{'cb-postprocess'}) ){
			...
			if( ! defined $pv ){
				# try YAML
				$pv = Data::Roundtrip::yaml2perl($remote_content);
			}
			if( ! defined $pv ){
				warn "warning, failed to parse received JSON or YAML data from URL '$aurl', I will save it as a Perl scalar/string.";
				$pv = $remote_content;
			}
		}
